...
# Initialize Spark Context with parallelism
conf = SparkConf().setAppName("Car Sales Analysis").setMaster("local[*]") \
    .set("spark.default.parallelism", "4")  # Adjust parallelism for better performance
...
rdd_with_year = rdd.map(extract_year).cache()  # Cache after the map operation
print("1. Map:")
...
filtered_rdd = rdd_with_year.filter(lambda row: row[YEAR_IDX] == "2022").cache()  # Cache filtered RDD
...
start_time = time.time()
transaction_count = filtered_rdd.count()  # Reuse the cached filtered_rdd
print("4. Count:")
